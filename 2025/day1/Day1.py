class Day1:
    def __init__(self,file_name:str) -> None:
        ...
        with open(file_name, 'r') as input_file:
            self._moves = map(lambda l: int(l[1:]) if l[0] == 'R' else int(l[1:])*-1,[line.strip('\n') for line in input_file])
    ...
    def parts1and2(self) -> tuple[int, int]:
        ...
        for move in self._moves:
            ...
            if dial_pos < 0:
                ...
                through_zero_count += dial_pos // self._dial_size
                
                # ended at zero then decrement by 1
                if dial_pos % self._dial_size == 0:
                    through_zero_count -= 1
            
            # set to dial new to new position
            dial_pos = dial_pos % self._dial_size
            
            # if at zero incrment zero count and set started_at_zero to True
            if dial_pos == 0:
                at_zero_count += 1
