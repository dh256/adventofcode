...
class Probe:
    ...
    def highest_y(self):
        ...
        highest_ever_y = 0
        for ix_vel in range(1,self.x_range.end+1):   # x_vel must be greater than 1 to move forward and <= x_range end to not overshoot target on first move 
            for iy_vel in range(self.y_range.start,self.x_range.end+1):   # y_vel must be greater than y_range start to not overshoot on first move - if outside a certain range will never hit target, this is just an arbitrary no just now. Should be able to calculate this
                highest_y = 0
                x, y = 0, 0
