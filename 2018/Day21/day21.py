...
class CPU():
    ...
    def run_program(self):
        with open("part2_results.txt", "w") as output:
            instructions=0
            first_time = True
            while self.ip < len(self.instructions):
                ...
                self.ip += 1
                if self.ip == 28: # and first_time:
                    print(f'{instructions},{self.registers[5]}')
                    output.write(f'{instructions},{self.registers[5]}\n')
                    output.flush()
                    #self.registers[0] = self.registers[5]
                    #self.ip = 0
                    #first_time = False
                instructions += 1
